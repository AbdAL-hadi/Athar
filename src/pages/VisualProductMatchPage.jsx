...
const VisualProductMatchPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!selectedFile) {
      setErrorMessage(t('visualMatch.uploadRequired', 'Please upload an image before searching for a match.'));
      return;
    }
    ...
    try {
        ...
        setUnavailableState({
          ...
          message:
            availabilityReason === 'no_catalog_products'
              ? t('visualMatch.catalogEmpty', 'No products are available in the store catalog right now.')
              : t('visualMatch.noCloseEnoughMessage', "Sorry, we could not find a close enough similar product in Athar's current collection."),
        });
        return;
      }
      ...
      if (!resultData?.product) {
        ...
        setErrorMessage(response?.message || t('visualMatch.missingProductResult', 'We could not compare this image against the catalog right now.'));
        return;
      }
      ...
      setErrorMessage(
        error?.message || t('visualMatch.requestFailed', 'We could not find a match right now. Please try again.'),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const matchNote = matchResult
    ? String(matchResult?.matchQuality || 'weak') === 'strong'
      ? t('visualMatch.strongMatch', 'We found a very similar product.')
      : String(matchResult?.matchQuality || 'weak') === 'medium'
        ? t('visualMatch.mediumMatch', 'We found a product with some similar visual details.')
        : t('visualMatch.weakMatch', "This is the closest available match in Athar's current collection.")
    : '';
      ...
      <section className="heritage-surface overflow-hidden rounded-[36px] border px-6 py-10 sm:px-8 lg:px-10">
        <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div className="max-w-3xl">
            <span className="heritage-pill">{t('visualMatch.pageLabel', 'Visual Product Match')}</span>
            <h1 className="mt-5 font-display text-4xl text-ink sm:text-5xl">
              {t('visualMatch.title', 'Find similar product')}
            </h1>
            <p className="mt-4 max-w-2xl text-lg leading-8 text-ink-soft">
              {t('visualMatch.description', 'Upload a photo of an item you like, and Athar will search for the closest match in our collection.')}
            </p>
          </div>

          <div className="heritage-panel rounded-[28px] px-5 py-5">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted">{t('visualMatch.howItWorks', 'How it works')}</p>
            <div className="mt-4 grid gap-3 text-sm leading-6 text-ink-soft sm:grid-cols-3 lg:grid-cols-1">
              <div className="rounded-[22px] bg-white/80 px-4 py-4">{t('visualMatch.step1', '1. Upload one clear product photo.')}</div>
              <div className="rounded-[22px] bg-white/80 px-4 py-4">{t('visualMatch.step2', '2. Athar checks visible colors, materials, and style.')}</div>
              <div className="rounded-[22px] bg-white/80 px-4 py-4">{t('visualMatch.step3', '3. We show the closest available item from the catalog.')}</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.02fr]">
        <section className="heritage-panel rounded-[32px] p-5 sm:p-6">
          <form className="space-y-5" onSubmit={handleSubmit}>
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted">{t('visualMatch.uploadProductImage', 'Upload product image')}</p>
              <h2 className="mt-2 font-display text-3xl text-ink">{t('visualMatch.referenceItem', 'Reference item')}</h2>
            </div>
            ...
            >
              {previewUrl ? (
                ...
                <div className="max-w-md">
                  ...
                  <p className="mt-5 text-lg font-semibold text-ink">{t('visualMatch.chooseReference', 'Choose a reference image')}</p>
                  <p className="mt-2 text-sm leading-6 text-ink-soft">
                    {t('visualMatch.imageHelp', 'JPG, PNG, or WEBP. Max 5MB.')}
                  </p>
                </div>
              )}
            </label>
            ...
            <div className="flex flex-wrap gap-3">
              <button type="submit" className="button-primary" disabled={!selectedFile || isLoading}>
                {isLoading ? t('visualMatch.analyzing', 'Analyzing your image...') : t('visualMatch.findSimilarProduct', 'Find similar product')}
              </button>
              <label htmlFor="visual-product-match-image" className="button-secondary cursor-pointer">
                {t('visualMatch.uploadImage', 'Upload image')}
              </label>
            </div>

            <div className="rounded-[22px] bg-white/70 px-4 py-4 text-sm leading-6 text-ink-soft">
              {t('visualMatch.focusTip', 'Focus on the item itself rather than the background. One main product in the frame works best.')}
            </div>
        ...
        <section className="heritage-panel rounded-[32px] p-5 sm:p-6">
          <div className="flex h-full flex-col">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted">{t('visualMatch.suggestion', 'Athar suggestion')}</p>
                <h2 className="mt-2 font-display text-3xl text-ink">{t('visualMatch.matchResult', 'Match result')}</h2>
              </div>

              {matchResult?.product ? (
                <div className="rounded-full bg-[#f4e4d4] px-4 py-2 text-sm font-semibold text-[#8f5f45]">
                  {t('visualMatch.similarity', '{{score}}% similarity', { score: scorePercentage })}
                </div>
              ) : null}
            </div>
            ...
            {!matchResult && !unavailableState && !isLoading ? (
              <div className="mt-6 flex flex-1 items-center justify-center rounded-[28px] border border-line bg-white/70 px-6 py-10 text-center">
                <p className="max-w-sm text-base leading-7 text-ink-soft">
                  {t('visualMatch.emptyPrompt', 'Upload a product image and Athar will look for the closest piece in the collection.')}
                </p>
              </div>
            ) : null}
              ...
              <div className="mt-6 flex flex-1 items-center justify-center rounded-[28px] border border-line bg-white/70 px-6 py-10 text-center">
                <div>
                  ...
                  <p className="mt-4 text-base text-ink-soft">
                    {t('visualMatch.loadingMessage', 'Athar is reviewing the image and comparing it with the catalog.')}
                  </p>
                </div>
              </div>
            ) : null}
              ...
              <div className="mt-6 flex flex-1 items-center rounded-[28px] border border-line bg-white px-6 py-8 shadow-card">
                <div className="max-w-xl">
                  ...
                  <h3 className="mt-4 font-display text-3xl text-ink">
                    {t('visualMatch.noCloseEnough', 'No similar product found')}
                  </h3>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    ...
                    <label htmlFor="visual-product-match-image" className="button-secondary cursor-pointer">
                      {t('common.uploadAnotherImage', 'Upload another image')}
                    </label>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 flex flex-1 flex-col rounded-[28px] bg-white p-4 shadow-card transition hover:-translate-y-1 hover:shadow-[0_24px_48px_rgba(120,84,60,0.18)]">
                {resolvedProductImage && !imageLoadFailed ? (
                  ...
                  <div className="flex aspect-[4/3] w-full items-center justify-center rounded-[22px] bg-[#f8eee7] px-6 text-center text-sm leading-6 text-ink-soft">
                    {t('visualMatch.imageUnavailable', 'Product image unavailable')}
                  </div>
                )}

                <div className="mt-5 flex flex-1 flex-col">
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-muted">
                    {t('visualMatch.scoreLine', 'Similarity score: {{score}}%', { score: scorePercentage })}
                  </p>
                  ...
                  <div className="mt-6 flex flex-wrap items-center gap-3">
                    <p className="text-sm text-ink-soft">
                      {matchedProduct.stock > 0 ? t('common.inStock', '{{count}} in stock', { count: matchedProduct.stock }) : t('common.outOfStock', 'Currently out of stock')}
                    </p>
                  </div>
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    ...
                    >
                      {t('visualMatch.viewProduct', 'View product')}
                    </button>
                    <label htmlFor="visual-product-match-image" className="button-secondary cursor-pointer">
                      {t('common.uploadAnotherImage', 'Upload another image')}
                    </label>
                  ...
                  <p className="mt-3 text-sm text-ink-soft">
                    {t('visualMatch.chooseAnother', 'Choose another reference image and click Find Similar Product to search again.')}
                  </p>
                </div>
              </div>
            ) : null}
